...
async def test_virtio_transport_mmio_disk_devices():
    ...
    root = Path(driver._work_dir) / "root"
    ...
    with patch("jumpstarter_driver_qemu.driver.Popen", side_effect=mock_popen):
        with patch("jumpstarter_driver_qemu.driver.run_process", side_effect=_mock_qemu_img_info(512)):
            with pytest.raises(RuntimeError, match="stop"):
                await power.on()
...
def test_tpm_properties():
    """TPM dir and socket paths should be under _work_dir."""
    driver = Qemu(tpm=True)
    assert driver._tpm_dir == Path(driver._work_dir) / "tpm"
    assert driver._tpm_socket == str(Path(driver._work_dir) / "tpm.sock")
